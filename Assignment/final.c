...
bool isLocalMaximum(double *local_data,int local_index, int NX, int NY, int NZ, int PX, int PY, int PZ,double *X_plus_ones_layer_recv,double *X_minus_ones_layer_recv,double *Y_plus_ones_layer_recv,double *Y_minus_ones_layer_recv,double *Z_plus_ones_layer_recv,double *Z_minus_ones_layer_recv,Point rankCoordinates){
    ...
    if(local_coordinates.x == 0 && rankCoordinates.x > 0){
        ...
        if(X_minus_ones_layer_recv[neighbour_index]>=local_data[local_index])
        return false;
    }

    if(local_coordinates.x == NX/PX-1 && rankCoordinates.x < PX-1){
        int neighbour_index = (NY/PY)*local_coordinates.z + local_coordinates.y;
        if(X_plus_ones_layer_recv[neighbour_index]>=local_data[local_index])
        return false;
    }

    if(local_coordinates.y == 0 && rankCoordinates.y > 0){
        int neighbour_index = (NX/PX)*local_coordinates.z + local_coordinates.x;
        if(Y_minus_ones_layer_recv[neighbour_index]>=local_data[local_index])
        return false;
    }

    if(local_coordinates.y == NY/PY-1 && rankCoordinates.y < PY-1){
        int neighbour_index = (NX/PX)*local_coordinates.z + local_coordinates.x;
        if(Y_plus_ones_layer_recv[neighbour_index]>=local_data[local_index])
        return false;
    }

    if(local_coordinates.z == 0 && rankCoordinates.z > 0){
        int neighbour_index = (NX/PX)*local_coordinates.y + local_coordinates.x;
        if(Z_minus_ones_layer_recv[neighbour_index]>=local_data[local_index])
        return false;
    }

    if(local_coordinates.z == NZ/PZ-1 && rankCoordinates.z < PZ-1){
        int neighbour_index = (NX/PX)*local_coordinates.y + local_coordinates.x;
        if(Z_plus_ones_layer_recv[neighbour_index]>=local_data[local_index])
        return false;
    }

    if(
        (local_coordinates.x == 0 || local_data[local_index] > local_data[coords_to_pos((Point){local_coordinates.x-1,local_coordinates.y,local_coordinates.z},NX/PX,NY/PY,NZ/PZ)])
        && (local_coordinates.x == NX/PX-1 || local_data[local_index]> local_data[coords_to_pos((Point){local_coordinates.x+1,local_coordinates.y,local_coordinates.z},NX/PX,NY/PY,NZ/PZ)])
        && (local_coordinates.y == 0 || local_data[local_index]> local_data[coords_to_pos((Point){local_coordinates.x,local_coordinates.y-1,local_coordinates.z},NX/PX,NY/PY,NZ/PZ)])
        && (local_coordinates.y == NY/PY-1 || local_data[local_index]> local_data[coords_to_pos((Point){local_coordinates.x,local_coordinates.y+1,local_coordinates.z},NX/PX,NY/PY,NZ/PZ)])
        && (local_coordinates.z == 0 || local_data[local_index]> local_data[coords_to_pos((Point){local_coordinates.x,local_coordinates.y,local_coordinates.z-1},NX/PX,NY/PY,NZ/PZ)])
        && (local_coordinates.z == NZ/PZ-1 || local_data[local_index]> local_data[coords_to_pos((Point){local_coordinates.x,local_coordinates.y,local_coordinates.z+1},NX/PX,NY/PY,NZ/PZ)])
    ){
        return true;
    }
    else {
...
bool isLocalMinimum(double *local_data,int local_index, int NX, int NY, int NZ, int PX, int PY, int PZ,double *X_plus_ones_layer_recv,double *X_minus_ones_layer_recv,double *Y_plus_ones_layer_recv,double *Y_minus_ones_layer_recv,double *Z_plus_ones_layer_recv,double *Z_minus_ones_layer_recv,Point rankCoordinates){
    Point local_coordinates = pos_to_coords(local_index, NX/PX, NY/PY, NZ/PZ);;
...
int main(int argc, char** argv) {
    ...
    for(int t=0;t<NC;t++){
        for(int i=0;i<elements_per_rank;i++){
            ...
            if(isLocalMinimum(local_data[t],i,NX,NY,NZ,PX,PY,PZ,X_plus_ones_layer_recv[t],X_minus_ones_layer_recv[t],Y_plus_ones_layer_recv[t],Y_minus_ones_layer_recv[t],Z_plus_ones_layer_recv[t],Z_minus_ones_layer_recv[t],rank_coordinates)){
                local_minimaCount[t]++;
            }
            if(isLocalMaximum(local_data[t],i,NX,NY,NZ,PX,PY,PZ,X_plus_ones_layer_recv[t],X_minus_ones_layer_recv[t],Y_plus_ones_layer_recv[t],Y_minus_ones_layer_recv[t],Z_plus_ones_layer_recv[t],Z_minus_ones_layer_recv[t],rank_coordinates)){
                local_maximaCount[t]++;
            }
    ...
    // reducing the local minima and maxima counts
    int global_minimaCount[NC], global_maximaCount[NC];
    double global_maximum[NC], global_minimum[NC];
    ...
    if(!rank){
        ...
        // writing the local minima and maxima counts
        for(int i = 0; i < NC; i++) {
            fprintf(output_file, "(%d, %d), ", global_minimaCount[i], global_maximaCount[i]);
        }
        fprintf(output_file, "\n");

        // writing the global minimum and maximum
        for(int i = 0; i < NC; i++) {
            fprintf(output_file, "(%lf, %lf), ", global_minimum[i], global_maximum[i]);
        }
        fprintf(output_file, "\n");

        // writing the read time, main code time and total time
